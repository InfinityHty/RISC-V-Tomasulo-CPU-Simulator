...
uint32_t ALU::Execute(const InsType type, const uint32_t data1, const uint32_t data2,const uint32_t imm,PC &pc) {
    ...
    if (type == and_) return data1 & data2;
    if (type == or_) return data1 | data2;
    if (type == xor_) return data1 ^ data2;
    ...
    if (type == addi) return data1 + imm;
    if (type == andi) return data1 & imm;
    if (type == ori) return data1 | imm;
    if (type == xori) return data1 ^ imm;
